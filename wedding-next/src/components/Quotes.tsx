...
export default function Quotes() {
  ...
  return (
    <section
      ...
      style={{
        transform: `perspective(1200px) rotateX(${(scrollProgress - 0.5) * 2}deg)`,
      ...
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {quotes.map((quote, index) => (
            <TiltCard
              key={index}
              className="text-center p-8 border border-[rgba(201,169,110,0.2)] rounded-xl bg-[var(--color-cream)] transition-all duration-400 hover:border-[var(--color-gold)] hover:shadow-[var(--shadow-soft)] animate-on-scroll"
            >
